class Locations {
    ...
    constructor(name: string, city: string, zipcode: number, address: any, img: string, date: any) {
        this.name = name;
        this.city = city;
...
class Restaurant extends Locations {
    ...
    constructor(name: string, city: string, zipcode: number, address: any, img: string, date: any, telephone: number, type: string, webaddress: string) {
...
let restaurants: Array<Restaurant> = [restaurant1, restaurant2, restaurant3, restaurant4];
for (let restaurant of restaurants) {
    (document.getElementById("restaurant") as HTMLElement).innerHTML += restaurant.display();
}
...
class Events extends Locations {
    ...
    constructor(name: string, city: string, zipcode: number, address: any, img: string, date: any, eventDate: any, eventTime: any, price: number) {
        super(name, city, zipcode, address, img, date)
...
let events: Array<Events> = [event1, event2, event3, event4];
for (let event of events) {
    (document.getElementById("event") as HTMLElement).innerHTML += event.display();
}
...
//Sort
let placesArrays: Array<Locations> = [place1, place2, place3, place4, restaurant1, restaurant2, restaurant3, restaurant4, event1, event2, event3, event4];
...
function ascending() {
    ...
    for (let placesArray of placesArrays) {
        ...
        (document.getElementById("ascending") as HTMLElement).innerHTML += placesArray.display();
    }
   
    (document.getElementById("placesCards") as HTMLElement).innerHTML = "";
    (document.getElementById("descending") as HTMLElement).innerHTML = "";

}

(document.getElementById("ascendingBtn") as HTMLElement).addEventListener("click", ascending);
...
function descending() {
    ...
    (document.getElementById("placesCards") as HTMLElement).innerHTML = "";
    (document.getElementById("ascending") as HTMLElement).innerHTML = "";
